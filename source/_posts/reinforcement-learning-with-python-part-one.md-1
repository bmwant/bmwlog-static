...
We can measure the results of the rollout and a result produced by another policy and in such way compare them. A good place to start here is always by providing some baseline for the return (some relative value we can compare later to). A **_random policy_** is usually used for that. Basically in each state driver will make its decisions by throwing a dice roll ([[1]](https://bmwlog.pp.ua/count-your-probabilities-using-python) [[2]](https://bmwlog.pp.ua/generating-events-to-count-probabilities-with-python)) and choosing an action based on that. Here's an implementation for the random policy.
